# apigw-lambda-aurora-crud-ts-sample-1
...
```bash
$ sam logs -n <<YourFunction>> --stack-name apigw-lambda-aurora-crud-ts-sample-1 --tail
```
...
```bash
sam delete --stack-name apigw-lambda-aurora-crud-ts-sample-1
```
